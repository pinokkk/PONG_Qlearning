...
class Paddle(pygame.Rect):
    ...
    def move_paddle(self, board_height, board_width, model, state, ball,va):
        ...
        if self.ia==0: #case where the paddle is controlled by a player
            ...
            if self.x == board_width-self.width :
                if ball.velocity > 0:
                    ...
                    if self.y + self.height/2 > board_height/2+5 :
                        self.y -= self.velocity
                    elif self.y + self.height/2 < board_height/2-5:
                        self.y += self.velocity
...
class Pong:
    ...
    # Création et chargement des poids du réseau de neurones
    ...
    model.load_weights('modeltrain11.h5')
    ...
    def next_step(self):
        ...
        state=self.getState()
        for paddle in self.paddles:
            paddle.move_paddle(self.HEIGHT,self.WIDTH, self.model, state, self.balls[0],self.visee_alea)
            pygame.draw.rect(self.screen, self.COLOUR, paddle)

        for ball in self.balls:
            if ball.velocity == 0 :
                if self.paddles[0].ia==0 or self.paddles[1].ia==0:
                    keys_pressed = pygame.key.get_pressed()
